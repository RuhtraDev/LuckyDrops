...
function updateCounters() {
    ...
    if (compsEl)
        compsEl.textContent = String(comps);
    ...
    if (ancientEl)
        ancientEl.textContent = String(ancient);
    if (rareCompEl)
        rareCompEl.textContent = String(rare);
    if (uncommonEl)
        uncommonEl.textContent = String(uncommon);
}
...
function isDuplicateDrop(itemName, chatLine, currentType) {
    ...
    if (dropsCache.has(cacheKey)) {
        ...
        debug(`🔍 Cache HIT: ${itemName} já registrado como ${cached === null || cached === void 0 ? void 0 : cached.type} às ${minuteKey}`);
        return true;
    }
...
function readChatbox() {
    ...
    for (let i = 0; i < opts.length; i++) {
        ...
        debug(`  Segment ${i}: y=${segment.y}, text="${(_a = segment.text) === null || _a === void 0 ? void 0 : _a.substring(0, 50)}"`);
    }
    ...
    for (let chatLine of allMessages) {
        ...
        if (!salvo && (chatLine.includes("Materials gained:") || chatLine.includes("Scavenging perk adds:"))) {
            ...
            for (let componentStr of componentsList) {
                ...
                let isCommon = COMMON_COMPONENTS.some(common => normalizedName.toLowerCase() === common.toLowerCase());
                if (isCommon) {
                    debug(`  ⏭️ Componente COMUM ignorado: ${normalizedName}`);
                    continue;
                }
                // Verificar tipo de componente
                let isAncient = ANCIENT_COMPONENTS.some(ancient => normalizedName.toLowerCase() === ancient.toLowerCase());
                let isRare = RARE_COMPONENTS.some(rare => normalizedName.toLowerCase() === rare.toLowerCase());
                let isUncommon = UNCOMMON_COMPONENTS.some(uncommon => normalizedName.toLowerCase() === uncommon.toLowerCase());
...
function showItems() {
    ...
    if (currentFilter !== "all") {
        if (currentFilter === "HSR") {
            filteredData = data.filter((item) => item.type.includes("HSR"));
        }
        else {
    ...
    function getEmojiByType(type, rarity) {
        if (type === "LOTD")
            return "💍";
    ...
    if (mode === "total") {
        ...
        filteredData.slice().reverse().forEach((item) => {
            ...
            itemList.insertAdjacentHTML("beforeend", `<li class="list-group-item item"${itemStyle} title="📅 ${dataFormatada} ${horaFormatada}">
                    ${displayText}
                </li>`);
        });
    }
...
function sendDiscordNotification(dropItem) {
    ...
    if (dropItem.type === "COMPS") {
        if (dropItem.isAncient)
            rarityText = " [Antigo]";
        else if (dropItem.isRare)
            rarityText = " [Raro]";
        else if (dropItem.isUncommon)
            rarityText = " [Incomum]";
    }
    const message = {
...
syncAllCounters();
debug(`✅ Inicializado com ${((_b = getSaveData("data")) === null || _b === void 0 ? void 0 : _b.length) || 0} drops`);
